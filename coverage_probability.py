...
def get_poisson_prob(test_stat, mean):
    ...
    # print '---' 
    # print '\t%s' % (mean**test_stat)
    # print '\t%s' % (math.exp(-mean))
    # print '\t%s' % (math.factorial(test_stat))
    # print '\t%s' % (math.exp(-mean)/(math.factorial(test_stat)))
    # print '\t%s' % (mean**test_stat * math.exp(-mean)/(math.factorial(test_stat)))
    prob = float(mean)**test_stat * math.exp(-float(mean))/float(math.factorial(test_stat))
    # print '\ttest stat: %s - mean: %s - prob: %s' % (test_stat, mean, prob)
    return prob
...
def get_cumulative_prob(test_stat, mean):
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    # print '========================================='
    # print 'cum prob of %s' % test_stat
    # prob = mean^test_stat * exp(-mean)/(test_stat)
    cum_prob = 0.
    t = 0
    while t <= test_stat:
        cum_prob += get_poisson_prob(t, mean)
        t += 1
    # print 'cum prob: %s' % cum_prob
    return cum_prob
...
def draw_profiles(l_profile, u_profile, sample_points):
    ...
    for i, key in enumerate(l_profile):
        ...
        frame = ROOT.TH2F( 'dummy_frame_profile__%s' % key
